...
object AchievementEngine {
    val definitions = listOf(
        ...
        AchievementDefinition("click_100000", rewardPrestigePoints = 2) { it.lifetimeStats.clicks >= 100_000 },
        ...
        ,AchievementDefinition("click_250k", rewardDebris = 5_000_000.0) { it.lifetimeStats.clicks >= 250_000 }
        ,AchievementDefinition("click_1m", rewardPrestigePoints = 4) { it.lifetimeStats.clicks >= 1_000_000 }
        ,AchievementDefinition("click_5m", rewardPrestigePoints = 10) { it.lifetimeStats.clicks >= 5_000_000 }
        ...
        ,AchievementDefinition("cases_250", rewardPrestigePoints = 3) { it.lifetimeStats.casesOpened >= 250 }
        ,AchievementDefinition("cases_500", rewardPrestigePoints = 5) { it.lifetimeStats.casesOpened >= 500 }
        ,AchievementDefinition("cases_1000", rewardPrestigePoints = 10) { it.lifetimeStats.casesOpened >= 1_000 }
        ,AchievementDefinition("events_100", rewardPrestigePoints = 3) { it.lifetimeStats.eventsCompleted >= 100 }
        ,AchievementDefinition("events_250", rewardPrestigePoints = 6) { it.lifetimeStats.eventsCompleted >= 250 }
        ,AchievementDefinition("events_500", rewardPrestigePoints = 10) { it.lifetimeStats.eventsCompleted >= 500 }
        ,AchievementDefinition("prestige_10", rewardPrestigePoints = 5) { it.lifetimeStats.prestiges >= 10 }
        ,AchievementDefinition("prestige_25", rewardPrestigePoints = 10) { it.lifetimeStats.prestiges >= 25 }
        ,AchievementDefinition("prestige_50", rewardPrestigePoints = 20) { it.lifetimeStats.prestiges >= 50 }
        ,AchievementDefinition("fleet_100", rewardPrestigePoints = 3) { it.fleetCounts.values.sum() >= 100 }
        ,AchievementDefinition("fleet_250", rewardPrestigePoints = 6) { it.fleetCounts.values.sum() >= 250 }
        ,AchievementDefinition("fleet_500", rewardPrestigePoints = 10) { it.fleetCounts.values.sum() >= 500 }
        ,AchievementDefinition("parts_25", rewardDebris = 2_000_000.0) { it.droneParts.values.sum() >= 25 }
        ,AchievementDefinition("parts_100", rewardPrestigePoints = 4) { it.droneParts.values.sum() >= 100 }
        ,AchievementDefinition("parts_300", rewardPrestigePoints = 8) { it.droneParts.values.sum() >= 300 }
        ...
        ,AchievementDefinition("planets_21", rewardPrestigePoints = 2) { it.ownedPlanets.size >= 21 }
        ,AchievementDefinition("planets_22", rewardPrestigePoints = 3) { it.ownedPlanets.size >= 22 }
        ,AchievementDefinition("planets_23", rewardPrestigePoints = 4) { it.ownedPlanets.size >= 23 }
